    ...
    -> std::expected<std::optional<Event>, QueryError>
{
    try
    {
        // The source-receiver distance and azimuth are sent as AQMS has
        // them - often null, since assocaro.delta is absent on 99.8% of
        // automatic rows.  The frontend computes the geometry itself when
        // it is missing, and flags that it did, which is a better place
        // for it: it knows what it drew.  See attic/arrivalGeometry for
        // the backend-side rectifier this replaced.
